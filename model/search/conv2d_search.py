...
from model.create.conv import create_model
from plot.graph import plot_model


def model_search(conv_layers=None, layer_sizes=None, activation_layer=None, shape=None,
                 dense_layers=None, output_classes=None, output_activation=None, X=None, y=None):

    if all(x is None for x in
           [conv_layers, layer_sizes, activation_layer, dense_layers, output_activation,
            output_classes]) and shape is not None and X is not None and y is not None:

        model_search(conv_layers=[1, 2, 3], layer_sizes=[32, 64, 128], activation_layer='relu', shape=shape,
        ...
        for dense_layer in dense_layers:
            for layer_size in layer_sizes:
                for conv_layer in conv_layers:
                    ...
                    model = create_model(conv_layers=conv_layer, conv_layer_size=layer_size,
                                         activation_layer=activation_layer,
                                         shape=shape, dense_layers=dense_layer, dense_layer_size=layer_size,
